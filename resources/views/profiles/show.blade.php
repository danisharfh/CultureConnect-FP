...
<div class="mt-2 mx-5 border" style="background: #FFFFFF; border-radius: 10px; border-color: Black; width: 800px;">
    <header class="mx-5 my-2 position-relative">
        ...
        <div class="position-relative mx-5">
            ...
            <img style="border-radius: 0.9em" src="{{$user->banner}}" height="250px" width="600px">
            ...
            <div class="d-flex align-self-center">
                ...
                @if(auth()->user()->isNot($user))
                    <form method="post" action="{{route('follow',$user->username)}}">
                        @csrf
                        <button type="submit"
                                class="btn btn-primary rounded-pill">{{auth()->user()->is_following_already($user) ? 'Unfollow': 'Follow'}}</button>
                    </form>
                @endif
            </div>
        </div>
        ...
        <div class="d-flex mx-5">
            ...
            <div class="d-flex">
                <h4 style="font-size: 14px">{{$user->follower->count()}}</h4>
